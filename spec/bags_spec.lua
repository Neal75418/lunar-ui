...
--------------------------------------------------------------------------------
-- ComputeBankLayoutSignature: fast-path cache fingerprint for RefreshBankLayout
--------------------------------------------------------------------------------

describe("ComputeBankLayoutSignature", function()
    local originalGetNumSlots
    ...
    before_each(function()
        ...
        LunarUI.db.profile.bags.slotSpacing = 4
    end)
    ...
    it("is stable across identical opens (fast-path enabling)", function()
        setSlotCounts({ [-1] = 28, [6] = 98, [7] = 98 })
        local sig1 = LunarUI._ComputeBankLayoutSignature()
        local sig2 = LunarUI._ComputeBankLayoutSignature()
        assert.equals(sig1, sig2, "signature must be deterministic for the same inputs")
    end)

    it("changes when slot counts change (cache-bust on tab purchase)", function()
        setSlotCounts({ [-1] = 28 })
        local sigBefore = LunarUI._ComputeBankLayoutSignature()
        setSlotCounts({ [-1] = 28, [6] = 98 }) -- new tab purchased
        local sigAfter = LunarUI._ComputeBankLayoutSignature()
        assert.not_equal(sigBefore, sigAfter)
    end)

    it("changes when viewport cols changes", function()
        setSlotCounts({ [-1] = 28 })
        local sigBefore = LunarUI._ComputeBankLayoutSignature()
        LunarUI.db.profile.bags.bankViewportCols = 16 -- user changed setting
        local sigAfter = LunarUI._ComputeBankLayoutSignature()
        assert.not_equal(sigBefore, sigAfter)
    end)

    it("changes when slot size changes (defensive against stale pixel geometry)", function()
        setSlotCounts({ [-1] = 28 })
        -- Reload bag constants so GetConstants() picks up new slotSize.
        -- LunarUI.BagsLoadSettings is exposed for this purpose.
        local sigBefore = LunarUI._ComputeBankLayoutSignature()
        LunarUI.db.profile.bags.slotSize = 42
        LunarUI.BagsLoadSettings()
        local sigAfter = LunarUI._ComputeBankLayoutSignature()
        assert.not_equal(sigBefore, sigAfter, "slot size change must bust the layout cache")
